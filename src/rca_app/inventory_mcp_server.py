...
def _get_config() -> AppConfig:
    if _inventory_config is None:
        raise RuntimeError(
            "Inventory MCP server not initialized. Call init_inventory_mcp_server(config)."
        )
    return _inventory_config


@inventory_mcp_server.tool()
...
def init_inventory_mcp_server(config: AppConfig) -> FastMCP:
    global _inventory_config
    _inventory_config = config
    logger.info("Inventory MCP server initialized")
    return inventory_mcp_server
